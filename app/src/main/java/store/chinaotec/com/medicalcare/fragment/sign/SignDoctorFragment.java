 ...
 * 签约医生页面
 */
public class SignDoctorFragment extends Fragment {
    ...
    public static SignDoctorFragment instance(String address) {
        Bundle bundle = new Bundle();
        SignDoctorFragment signDoctorFragment = new SignDoctorFragment();
        bundle.putString("address", address);
        signDoctorFragment.setArguments(bundle);
        return signDoctorFragment;
    }

    @Override
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        cityArr = getArguments().getString("address");
        View view = inflater.inflate(R.layout.fragment_sign_doctor, container, false);
    ...
    private void getDoctorData() {
        ...
        OkHttpUtils.post().url(MyUrl.SIGN_DOCTOR_LIST).params(mParams).build().execute(new StringCallback() {
            @Override
    ...
    class DoctorRecyclerAdapter extends RecyclerView.Adapter<DoctorRecyclerAdapter.ViewHolder> {
        ...
        public void onBindViewHolder(ViewHolder holder, int position) {
            ...
            Glide.with(getActivity()).load(doctor.getHeadImage()).fitCenter().transform(new GlideRoundTransform(getActivity(), 5)).placeholder(R.mipmap.online_doctor_logo).into(holder.signDoctorPhoto);
