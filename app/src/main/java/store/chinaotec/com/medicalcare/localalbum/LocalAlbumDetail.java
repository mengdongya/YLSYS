    ...
    public void onClick(View view) {
        switch (view.getId()) {
            ...
            case R.id.header_finish:
                AppManager.getInstance().killActivity(LocalAlbum.class);
                ...
                runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                        if (folders != null) {
                            ...
                            IconDetailAdapter adapter = new IconDetailAdapter(LocalAlbumDetail.this, folders);
                            title.setText(folder);
                            gridView.setAdapter(adapter);
                            //设置当前选中数量
                            if (checkedItems.size() + LocalImageHelper.getInstance().getCurrentSize() > 0) {
                                finish.setText("完成(" + (checkedItems.size() + LocalImageHelper.getInstance().getCurrentSize()) + "/6)");
                                finish.setEnabled(true);
                                headerFinish.setText("完成(" + (checkedItems.size() + LocalImageHelper.getInstance().getCurrentSize()) + "/6)");
...
//                                headerFinish.setEnabled(false);
    ...
    public void onCheckedChanged(CompoundButton compoundButton, boolean b) {
        if (!b) {
            ...
            if (!checkedItems.contains(compoundButton.getTag())) {
                if (checkedItems.size() + LocalImageHelper.getInstance().getCurrentSize() >= 6) {
                    Toast.makeText(this, "最多选择6张图片", Toast.LENGTH_SHORT).show();
    ...
    public class IconDetailAdapter extends BaseAdapter {
        ...
        public IconDetailAdapter(Context context, List<LocalImageHelper.LocalFile> paths) {
            m_context = context;
            this.paths = paths;
        }
        ...
        public View getView(final int i, View convertView, ViewGroup viewGroup) {
            ...
            Glide.with(LocalAlbumDetail.this).load(localFile.getThumbnailUri()).placeholder(R.drawable.error_img).into(imageView);
