...
//        activity.startActivityForResult(IntentUtils.getLaunchAppIntent(packageName), requestCode);
        String phone = SpUtill.getString(MyApp.getContext(), ResourseSum.Medica_SP, "userPhone");
        ...
        bundle.putString("telNumber", phone);
        intent.putExtras(bundle);
        //参数1：要调用另一个APP的activity所在的包名
        //参数2：要调用另一个APP的activity名字
        intent.setClassName(packageName, "com.evmtv.microvideocloud.MainActivity");
        activity.startActivityForResult(intent, 1);
    }
    //在启动前进行一次判断：app是否在后台运行。一下是判断app是否运行的方法：
    private static boolean isBackgroundRunning(Activity activity) {
        String processName = MyCommonUtil.PACKAGENAME;
    ...
    public static boolean isAppForeground() {
        ActivityManager manager = (ActivityManager) Utils.getContext().getSystemService(ACTIVITY_SERVICE);
